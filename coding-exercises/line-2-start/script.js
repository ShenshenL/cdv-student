...
function gotData(incomingData){
  ...
  let u = [];
  let c = [];
  u.push(incomingData[0])
  c.push(incomingData[1])

  console.log(u);
  console.log(c);

  function usa() {
  graphGroup.selectAll('.line').data(u).enter()
    .append('path')
      .attr('d', lineMaker)
      .attr('fill', 'none')
      .attr('stroke', 'blue')
    ...
    graphGroup.selectAll('.line').data(u).transition().duration(500)
      .attr("d", lineMaker)
      .attr("stroke", "blue");

}
d3.select("#usa").on("click", usa)

function china() {
  graphGroup.selectAll('.line').data(c).enter()
    .append('path')
      .attr('d', lineMaker)
      .attr('fill', 'none')
      .attr('stroke', 'blue')
  ...
  graphGroup.selectAll('.line').data(c).transition().duration(500)
    .attr("d", lineMaker)
    .attr("stroke", "red");

}
d3.select("#china").on("click", china)
